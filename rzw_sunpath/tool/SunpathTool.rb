  ...
  module Sunpath
    class SunpathTool
      ...
      def activate
        ...
        @window = create_window if @window.nil? || !@window.visible?
        show_dlg(@window)
        Sketchup.active_model.active_view.invalidate
      end

      def deactivate(view)
        destroy_dlg(@window) if @window
        model = Sketchup.active_model
        shadow_info = model.shadow_info
        shadow_info["DisplayShadows"] = false
        view.invalidate
      ...
      def draw(view)
        @ip.draw(view)
        view.line_width = 3
        view.draw_points([ip2pt_z0(@ip)], 10, 4, OPT_PREVIEW_COLOR)
      ...
      def create_window
        ...
        window.set_file(HTML_DIALOG_SUNPATH)

        window.add_action_callback("ready") do |_action_context|
          init_window
        end

        window.add_action_callback("update_scene") do |_action_context|
          Sunpath.operation('Update Scene') { update_scene }
        end

        window.add_action_callback("domeSizeChange") do |_action_context, scale|
          @scale_sun_path = Sp.nil_or_set(scale, :to_f)
          Sketchup.active_model.active_view.invalidate
        end

        window.add_action_callback("callback_time") do |_action_context, month, day, hour|
          @month = month.to_i
          @day = day.to_i
          @hour = hour.to_i
        ...
        window.add_action_callback("submit_location") do |_action_context, info_json|
          info = info_json.is_a?(String) ? JSON.parse(info_json) : info_json
      ...
      def init_window
        @window.execute_script("vm_local.city = #{js_string(@city)}")
        @window.execute_script("vm_local.longitude = #{@lon.to_f}")
        @window.execute_script("vm_local.latitude = #{@lat.to_f}")

        @window.execute_script('sp.gen_time_step()')
        init_window_time
        @window.execute_script("vm_dome_scale.scale = #{@scale_sun_path.to_f}")
        init_window_library
      end
      ...
      def redraw_position_sun
        ...
        Sunpath.operation('重绘太阳', true) do
          ...
          hoys.each { |hoy|
            ...
            [7.5, 22.5, 37.5, 52.5].each { |minute| # todo 要全部错位一半 [7.5, 22.5, 37.5, 52.5]
              sunpath.zenith, sunpath.solAz, sunpath.solAlt =
                Sunpath.noaaSolarCalculator(latitude, longitude, time_zone, m, d, h, minute).map { |val| val.degrees }
              if sunpath.solAlt > 0
                grp_sun, vec_sun, pt_sun = sunpath.get_pt_sun(sc_sun)
                pt_sunL << pt_sun
              end
