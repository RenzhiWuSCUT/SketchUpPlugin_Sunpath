module SuperCat
  module Sunpath
    ...
    def self.noaaSolarCalculator(latitude, longitude, timeZone, month, day, hour, minute = 0)
      # by NOAA Earth System Research Laboratory
      # NOAA defines longitude and time zone as positive to the west:
      timeZone = -timeZone
      longitude = -longitude
      # >>>> Purpose【doy = 儒略日】
      doy = Sunpath.getJD(month, day).to_i
      # minute = 0 # default
      second = 0 # default
      # Purpose: 【求解gamma】
      # >>>> Purpose【gamma = fractional year = 分数年】
      # >>>> Idea【gamma = f(doy,hour)】
      gamma = (2 * Math::PI) / 365 * (doy - 1 + ((hour - 12).to_f / 24.to_f).round)
      # Purpose: 【求解eqtime和decl】
      # >>>> Purpose【eqtime = equation of time = 等效时间（分钟制）】
      # >>>> Idea【eqtime = f(gamma)】
      ...
      # >>>> Purpose【declAngle = solar declination angle = 太阳赤道角（radians）】
      # >>>> Idea【decl = f(gamma)】
      ...
      # Purpose: 【求解hangle；根据eqtime】
      # >>>> Purpose【time_offset = 时间偏移量（radians）】
      # >>>> Idea【time_offset = f(eqtime,longitude,timeZone)】
      time_offset = eqtime - 4 * longitude + 60 * timeZone
      # >>>> Purpose【tst = true solar time = 真太阳时（分钟制）】
      # >>>> Idea【hangle = f(hour,minute,second,time_offset)】
      tst = hour * 60 + minute + second / 60 + time_offset
      # >>>> Purpose【solarHangle = solar hour angle = 太阳时角】
      # >>>> Idea【hangle = f(tst)】
      solarHangle = (tst / 4) - 180
      # Purpose: 【求解[solarZenithD, solarAzimuthD, solarAltitudeD]；根据hangle,lat,decl】
      # >>>> Purpose【solarZenith = solar zenith angle = 太阳天顶角】
      # >>>> Idea【zenith = f(hangle,lat,decl)】
      solarZenithR = Math.acos(Math.sin(latitude.degrees) * Math.sin(declAngle) + Math.cos(latitude.degrees) * Math.cos(declAngle) * Math.cos(solarHangle.degrees))
      solarZenithD = solarZenithR.radians
      solarZenithD = 90 if solarZenithD > 90
      solarZenithD = 0 if solarZenithD < 0
      # >>>> Purpose【solarAltitude = solar altitude angle = 太阳高度角】
      # >>>> Idea【altitude = f(zenith)】
      solarAltitudeD = 90 - solarZenithD
      # >>>> Purpose【solarAzimuth = solar azimuth angle = 太阳方位角】
      # >>>> Idea【altitude = f(zenith,lat,decl)】
      solarAzimuthR = -(Math.sin(latitude.degrees) * Math.cos(solarZenithR) - Math.sin(declAngle)) / (Math.cos(latitude.degrees) * Math.sin(solarZenithR))
      solarAzimuthR = Math.acos(solarAzimuthR)
      # Modify【2021-07-12 18:54:26】【增加一个决定solarAzimuthR正负号的判别】
      if solarHangle == 0.0 || solarHangle == -180.0 || solarHangle == 180.0
        solarAzimuthR = (declAngle < latitude.degrees) ? Math::PI : 0.0
      else
        solarAzimuthR =
          if solarHangle > 0
            ((Math.acos(((Math.sin(latitude.degrees) * Math.cos(solarZenithR)) - Math.sin(declAngle)) / (Math.cos(latitude.degrees) * Math.sin(solarZenithR))) + Math::PI) % (2 * Math::PI))
          else
            ((3 * Math::PI - Math.acos(((Math.sin(latitude.degrees) * Math.cos(solarZenithR)) - Math.sin(declAngle)) / (Math.cos(latitude.degrees) * Math.sin(solarZenithR)))) % (2 * Math::PI))
          end
      end

      solarAzimuthD = solarAzimuthR.radians


      [solarZenithD, solarAzimuthD, solarAltitudeD]
